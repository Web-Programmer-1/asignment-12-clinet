...
const Navbar = () => {
    ...
    useEffect(() => {
        const dUser = users.find((u) => u.email === user?.email);
        setRole(dUser?.role);
    }, [users, user]);
    ...
    return (
        <div className="fixed top-0 z-50 w-full h-16 backdrop-blur-md bg-teal-100">
            ...
            <div className="w-11/12 mx-auto h-full flex justify-between items-center">
                {/* Left Logo */}
                <div>
                    <Link to="/" className="text-2xl md:text-4xl font-logoFont font-extrabold">
                        <b className="text-orange-500">U</b>ni
                        <b className="text-orange-500">G</b>rants
                    </Link>
                </div>
                ...
                <div className="flex-1 hidden lg:block">
                    <div className="flex w-full items-center justify-end lg:text-lg font-logoFont">
                        <NavLink className="hover:bg-orange-400 hover:text-white rounded-sm px-3 py-2" to="/">
                            Home
                        </NavLink>
                        <NavLink className="hover:bg-orange-400 hover:text-white rounded-sm px-3 py-2" to="/all_blogs">
                            Blogs
                        </NavLink>
                        <NavLink className="hover:bg-orange-400 hover:text-white rounded-sm px-3 py-2" to="/how_to_apply">
                            How To Apply
                        </NavLink>
                        <NavLink className="hover:bg-orange-400 hover:text-white rounded-sm px-3 py-2" to="/all-scholarship">
                            All Scholarship
                        </NavLink>
                        <NavLink className="hover:bg-orange-400 hover:text-white rounded-sm px-3 py-2" to="/about">
                            About
                        </NavLink>
                        {user?.email && role === "user" && (
                            <NavLink className="hover:bg-orange-400 hover:text-white rounded-sm px-3 py-2" to="/user-dashboard">
                                Dashboard
                            </NavLink>
                        )}
                        {user?.email && role === "Admin" && (
                            <NavLink className="hover:bg-orange-400 hover:text-white rounded-sm px-3 py-2" to="/admin-dashboard">
                                Admin Dashboard
                            </NavLink>
                        )}
                        {user?.email && role === "Moderator" && (
                            <NavLink className="hover:bg-orange-400 hover:text-white rounded-sm px-3 py-2" to="/mod-dashboard">
                                Moderator Dashboard
                            </NavLink>
                        )}
                        {user?.email ? (
                            <>
                                <div
                                    onClick={handleLogOut}
                                    className="hover:bg-orange-400 hover:text-white rounded-sm px-3 py-2 cursor-pointer"
                                >
                                    Logout
                                </div>
                                <div className="w-12 h-12 ml-4 rounded-full border border-orange-500">
                            ...
                            <div className="ml-4 flex items-center gap-2">
                                <Link
                                    ...
                                    onMouseLeave={() => setHover(false)}
                                    to="/login"
                                    className="px-3 py-2 rounded-sm hover:bg-orange-500 border border-orange-500 hover:text-white"
                                >
                                    Login
                                </Link>
                                <Link
                                    to="/register"
                                    className={`px-3 py-2 rounded-sm border border-orange-500 ${
                                        hover ? "bg-white text-black" : "bg-orange-500 text-white"
                                    }`}
                                >
                                    Register
                                </Link>
                ...
                <div className="lg:hidden flex items-center gap-2">
                    ...
                    <div
                        ...
                        className="w-12 h-12 rounded-md bg-[rgba(0,0,0,0.3)] flex justify-center items-center"
                    >
                        {!mobileMenu ? (
                            <GiHamburgerMenu className="text-3xl text-orange-500" />
                        ) : (
                            <GiCrossedBones className="text-3xl text-red-500" />
                        )}
                    </div>
                </div>
            </div>
            ...
            {/* Mobile Menu Links */}
            {mobileMenu && (
                <div className="w-full py-10 flex justify-center backdrop-blur-lg bg-gradient-to-b z-50 from-teal-200 to-indigo-500">
                    <div className="w-11/12 mx-auto">
                        <div className="flex flex-col text-lg font-logoFont gap-2">
                            <NavLink
                                onClick={() => setMobileMenu(false)}
                                className="hover:bg-orange-400 hover:text-white rounded-sm px-3 py-2"
                                to="/"
                            >
                                Home
                            </NavLink>
