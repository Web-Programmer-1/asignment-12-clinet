...
const TopScholarship = () => {
    ...
    // নিশ্চিত করছি যে scholarship একটি বৈধ অ্যারে কিনা
    const validScholarships = Array.isArray(scholarship) ? scholarship : [];
    ...
    return (
        <div className="w-full bg-gradient-to-b from-blue-50 via-teal-100 font-logoFont to-blue-50">
            ...
            <div className="w-11/12 mx-auto">
                <div className="w-full md:grid md:grid-cols-2 lg:grid-cols-3 gap-5 space-y-5 md:space-y-0 ">
                    {
                        validScholarships.slice(0, 6).map((item, idx) => (
                            <S_Card data={item} key={idx}></S_Card>
                        ))
                    }
                </div>
